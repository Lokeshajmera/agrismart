...
import { Bell, User, Menu, Settings, LogOut, Map, AlertTriangle, Droplets, ThermometerSun, Search, Languages, Leaf, ChevronDown, Wheat, IndianRupee, Sun, Moon } from 'lucide-react';
...
export default function Navbar({ setIsMobileMenuOpen }) {
    ...
    return (
        <header className="bg-white dark:bg-nature-950/80 backdrop-blur-md border-b border-nature-200 dark:border-nature-800 h-20 flex items-center justify-between px-6 lg:px-8 shrink-0 relative z-[100]">
            <div className="flex items-center gap-4 lg:hidden">
                <button
                    onClick={() => setIsMobileMenuOpen?.(prev => !prev)}
                    className="text-nature-600 hover:text-nature-900 dark:text-white transition-colors cursor-pointer p-1"
                >
                    <Menu className="w-6 h-6" />
                </button>
                <div className="font-bold text-lg text-nature-800 dark:text-nature-100 flex items-center gap-2 tracking-tight">
                                    ...
                                    {filteredSuggestions.map((suggestion, idx) => (
                                        <li key={idx}>
                                            <button
                                                ...
                                                className="w-full text-left px-4 py-2.5 hover:bg-nature-50 dark:bg-nature-900 transition-colors flex items-center gap-3 text-sm text-nature-700 dark:text-nature-200 hover:text-earth-600 cursor-pointer"
                                            >
            ...
            <div className="flex items-center gap-3">
                ...
                <button
                    onClick={toggleTheme}
                    className="p-2 text-nature-600 hover:bg-nature-50 dark:bg-nature-900 rounded-full transition-colors flex items-center gap-1 cursor-pointer"
                ...
                <div className="relative" ref={langRef}>
                    <button
                        ...
                        className="p-2 text-nature-600 hover:bg-nature-50 dark:bg-nature-900 rounded-full transition-colors flex items-center gap-1"
                    >
                        <Languages className="w-5 h-5" />
                        <span className="text-xs font-bold uppercase">{i18n.language.split('-')[0]}</span>
                    </button>
                    {showLangMenu && (
                        <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-nature-950 rounded-xl shadow-lg border border-nature-200 dark:border-nature-800 py-2 z-50">
                            {[
                                ...
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2 text-sm hover:bg-nature-50 dark:bg-nature-900 ${i18n.language.startsWith(lang.code) ? 'text-earth-600 font-bold' : 'text-nature-700 dark:text-nature-200'}`}
                                >
                                    {lang.label}
                                </button>
                            ))}
                ...
                <div className="relative ml-2" ref={menuRef}>
                    <button
                        ...
                        className="flex items-center gap-3 pl-4 border-l border-nature-200 dark:border-nature-800 hover:bg-nature-50 dark:bg-nature-900 rounded-xl py-1.5 pr-2 transition-colors cursor-pointer"
                    >
                        ...
                        <div className="w-10 h-10 rounded-full bg-nature-100 dark:bg-nature-800 overflow-hidden border border-nature-200 dark:border-nature-800 flex items-center justify-center text-nature-500">
