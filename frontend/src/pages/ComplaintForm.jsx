...
import { MessageSquare, Send, AlertCircle, Clock, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="mt-8 mb-12">
        <h2 className="text-xl font-bold text-nature-900 dark:text-white mb-4">My Complaints</h2>
        {complaints.length === 0 ? (
          <div className="bg-nature-50 dark:bg-nature-900/50 rounded-xl p-8 text-center border border-nature-200 dark:border-nature-800">
            <MessageSquare className="w-12 h-12 text-nature-300 dark:text-nature-600 mx-auto mb-3" />
            <p className="text-nature-600 dark:text-nature-400">You haven't submitted any complaints yet.</p>
          </div>
        ) : (
            ...
            {complaints.map((c) => (
              <div key={c.id} className="bg-white dark:bg-nature-950 p-5 rounded-xl border border-nature-200 dark:border-nature-800 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-start animate-in fade-in zoom-in-95 duration-300">
                <div className="flex-1">
                  ...
                  <p className="text-nature-600 dark:text-nature-400 text-sm mb-3 max-w-2xl leading-relaxed">{c.message}</p>
                  <p className="text-xs text-nature-400 dark:text-nature-500 font-medium">{new Date(c.created_at).toLocaleString()}</p>
                </div>
                
                <div className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border ${c.status === 'resolved' ? 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:border-green-800/50 dark:text-green-400' : 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800/50 dark:text-amber-400'}`}>
