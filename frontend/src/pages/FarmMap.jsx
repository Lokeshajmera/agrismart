...
export default function FarmMap() {
 ...
 <div>
 <h1 className="text-2xl font-bold text-nature-900 dark:text-white tracking-tight">{tLive("Interactive Farm Map")}</h1>
 <p className="text-nature-500 dark:text-white mt-1">{tLive("Real-time geospatial data, NDVI layers, and sensor locations.")}</p>
 </div>
 ...
 <span className="text-sm font-bold text-white tracking-wide">{tLive("Farm Boundary")}</span>
 </div>
 <div className="flex items-center gap-3">
 <div className="w-4 h-4 bg-orange-500/40 border-2 border-orange-500 rounded-full shadow-[0_0_10px_rgba(249,115,22,0.5)]"></div>
 <span className="text-sm font-bold text-white tracking-wide">{tLive("Dry Zone Detected")}</span>
 </div>
 <div className="flex items-center gap-3">
 ...
 <span className="text-sm font-bold text-white tracking-wide">{tLive("Live IoT Sensor")}</span>
 </div>
 </div>
 ...
 <TileLayer
 attribution='&copy; <a href="https://www.openstreetmap.org/copyright">{tLive("OpenStreetMap")}</a> contributors'
 ...
 <p className="text-sm">{tLive("Type:")} <span className="font-medium">{sensor.type}</span></p>
 <p className="text-sm">{tLive("Value:")} <span className={`font-bold ${sensor.status === 'warning' ? 'text-orange-500' : sensor.status === 'critical' ? 'text-red-600' : 'text-green-600'}`}>{sensor.val}</span></p>
 <p className="text-xs text-nature-400 dark:text-white mt-2">{tLive("Last updated: Just now")}</p>
 </div>
 </Popup>
 </Marker>
 ))}
